...
public class WebSocketTest {
    ...
    public static ConcurrentHashMap<String, WebSocketTest> webSocketSet = new ConcurrentHashMap<String, WebSocketTest>();
    ...
    public void onOpen(@PathParam(value = "userno") String param, Session WebSocketsession, EndpointConfig config) {
        ...
        sendAlll(param+"已加入聊天"+",当前在线人数为："+getOnlineCount());//2018.7.8   当新用户加入界面显示用户加入和聊天人数       
    }
    ...
    public void onClose() {
        if (!userno.equals("")) {
            ...
            sendAlll(userno+"已退出聊天"+",当前在线人数为："+getOnlineCount());//2018.7.8   当用户推出界面显示用户推出和聊天人数 
    ...
    public void sendAlll(String message) {
        ...
        for (String key : webSocketSet.keySet()) {
            try {
                //判断接收用户是否是当前发消息的用户
                if (!userno.equals(key)) {
                    webSocketSet.get(key).sendMessage(now+sendMessage);
